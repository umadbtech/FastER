...
class ProfileRepository(private val profileApiService: ProfileApiService) {
     ...
     */
    fun saveLegalName(firstName: String, lastName: String, accessToken: String): Flow<Result<ProfileSummary>> = flow {
        try {
            val request = SaveLegalNameRequest(firstName, lastName)
            val response = profileApiService.saveLegalName(
                authorization = "Bearer $accessToken",
                request = request
            )
    ...
    fun uploadAvatar(imageFile: File, accessToken: String): Flow<Result<String>> = flow {
        try {
            ...
            val response = profileApiService.uploadAvatar(
                authorization = "Bearer $accessToken",
                file = part
            )
    ...
    fun getAvatarUrl(accessToken: String): Flow<Result<String>> = flow {
        try {
            val response = profileApiService.getAvatarUrl(
                authorization = "Bearer $accessToken"
            )
    ...
    fun saveEmergencyContact(
        ...
        try {
            ...
            val response = profileApiService.saveEmergencyContact(
                authorization = "Bearer $accessToken",
                request = request
            )
     ...
     */
    fun deleteEmergencyContact(contactId: String, accessToken: String): Flow<Result<ProfileSummary>> = flow {
        try {
            val response = profileApiService.deleteEmergencyContact(
                authorization = "Bearer $accessToken",
                contactId = contactId
            )
